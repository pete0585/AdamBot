#!/usr/bin/env bash
# Update an existing CMS item in a Webflow collection
# Usage: update-collection-item.sh <collection-id> <item-id> <json-file>
set -euo pipefail

COLLECTION_ID="${1:?Error: collection-id is required as first argument}"
ITEM_ID="${2:?Error: item-id is required as second argument}"
JSON_FILE="${3:?Error: json-file path is required as third argument}"
